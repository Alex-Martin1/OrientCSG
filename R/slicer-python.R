...
emit_slicer_section_python <- function(res, section = NULL) {
  ...
  code <- c(
    ...
    "",
    sprintf("CAMERA_DISTANCE = %s", fmt_num_py(res$camera_distance)),
    "CAMERA_POSITION_DISTANCE_MM = 300.0",
    "BASE_PARALLEL_SCALE_MM = 35.0",
    ...
    "    camera.SetParallelScale(BASE_PARALLEL_SCALE_MM * CAMERA_DISTANCE)",
    ...
    "camera = set_camera(PSEC, Xcam, Ycam, Zcam)",
...
emit_slicer_longbone_volume_python <- function(res, section = NULL) {
  ...
  code <- c(
    ...
    "# Display controls",
    sprintf("CAMERA_DISTANCE = %s", fmt_num_py(res$camera_distance)),
    "BASE_FIELD_OF_VIEW_Y_MM = 70.0",
    "BASE_PARALLEL_SCALE_MM = 35.0",
    "CAMERA_POSITION_DISTANCE_MM = 300.0",
    "ORIENT_3D_CAMERA = True",
    "VIEW_FROM_PROXIMAL = True",
    sprintf("ANTERIOR_UP_SIGN = %s", fmt_num_py(anterior_up_sign)),
    sprintf("ML_RIGHT_SIGN = %s", fmt_num_py(ml_right_sign)),
    "RULER_COLOR = 0",
    ...
    sprintf("PROXIMAL_AXIS_POINT = %s", fmt_py_vec(proximal_ras)),
    "",
    "ORIENTCSG_INITIAL_SLICE_TO_RAS = None",
    "ORIENTCSG_INITIAL_VOLUME_NODE_ID = None",
    "",
    "def nrm(v):",
    "    v = np.asarray(v, dtype=float)",
    "    s = np.linalg.norm(v)",
    ...
    "        fov_y = BASE_FIELD_OF_VIEW_Y_MM * CAMERA_DISTANCE",
    "        fov_x = fov_y * aspect",
    "        fov_z = float(sliceNode.GetFieldOfView()[2])",
    "        sliceNode.SetFieldOfView(float(fov_x), float(fov_y), float(fov_z))",
    ...
    "        return None",
    "",
    "def configure_3d_view(threeDView):",
    "    viewNode = threeDView.mrmlViewNode()",
    "    viewNode.SetBackgroundColor(0.0, 0.0, 0.0)",
    "    viewNode.SetBackgroundColor2(0.0, 0.0, 0.0)",
    "    viewNode.SetBoxVisible(False)",
    ...
    "    # L is emitted by R as an anatomical distal-to-proximal vector.",
    "    # A proximal view places the camera on the proximal side of the section,",
    "    # matching the long-bone model-based Slicer workflow.",
    "    Zcam = nrm(L)",
    "    if not VIEW_FROM_PROXIMAL:",
    "        Zcam = -Zcam",
    "    desired_up = ANTERIOR_UP_SIGN * AP",
    ...
    "    return Xcam, Ycam, Zcam",
    "",
    "def set_3d_camera(Psec, Xcam, Ycam, Zcam):",
    "    threeDView = slicer.app.layoutManager().threeDWidget(0).threeDView()",
    "    cameraNode = threeDView.cameraNode()",
    "    camera = cameraNode.GetCamera()",
    "    configure_3d_view(threeDView)",
    "    C = Psec + CAMERA_POSITION_DISTANCE_MM * Zcam",
    "    camera.SetPosition(float(C[0]), float(C[1]), float(C[2]))",
    "    camera.SetFocalPoint(float(Psec[0]), float(Psec[1]), float(Psec[2]))",
    "    camera.SetViewUp(float(Ycam[0]), float(Ycam[1]), float(Ycam[2]))",
    ...
    "    return camera",
    "",
    "def save_camera_state(volumeNode, camera):",
    "    volumeNode.SetAttribute('OrientCSG.CameraPosition', ','.join([f'{v:.12g}' for v in camera.GetPosition()]))",
    "    volumeNode.SetAttribute('OrientCSG.CameraFocalPoint', ','.join([f'{v:.12g}' for v in camera.GetFocalPoint()]))",
    "    volumeNode.SetAttribute('OrientCSG.CameraViewUp', ','.join([f'{v:.12g}' for v in camera.GetViewUp()]))",
    "    volumeNode.SetAttribute('OrientCSG.CameraParallelScale', f'{camera.GetParallelScale():.12g}')",
    "",
    "def get_initial_volume_node():",
    "    if ORIENTCSG_INITIAL_VOLUME_NODE_ID:",
    "        node = slicer.mrmlScene.GetNodeByID(ORIENTCSG_INITIAL_VOLUME_NODE_ID)",
    "        if node is not None:",
    "            return node",
    ...
    "    cameraNode = threeDView.cameraNode()",
    "    camera = cameraNode.GetCamera()",
    "    pos_txt = volumeNode.GetAttribute('OrientCSG.CameraPosition')",
    "    foc_txt = volumeNode.GetAttribute('OrientCSG.CameraFocalPoint')",
    "    up_txt = volumeNode.GetAttribute('OrientCSG.CameraViewUp')",
    "    scale_txt = volumeNode.GetAttribute('OrientCSG.CameraParallelScale')",
    "    if None in [pos_txt, foc_txt, up_txt, scale_txt]:",
    "        raise ValueError('No saved OrientCSG camera state found.')",
    "    pos = [float(x) for x in pos_txt.split(',')]",
    "    foc = [float(x) for x in foc_txt.split(',')]",
    ...
    "    threeDView.forceRender()",
    "",
    "def orient_3d_camera(volumeNode):",
    "    if not ORIENT_3D_CAMERA:",
    "        return None",
    "    Xcam, Ycam, Zcam = compute_camera_basis(L, ML, AP)",
    "    camera = set_3d_camera(PSEC, Xcam, Ycam, Zcam)",
    "    save_camera_state(volumeNode, camera)",
    "    return camera",
    "",
    "def add_axis_line(name, p1, p2, color=(1.0, 1.0, 1.0)):",
    "    lineNode = slicer.mrmlScene.AddNewNodeByClass('vtkMRMLMarkupsLineNode', name)",
    ...
    "    fov_x = float(fov[0])",
    "    fov_y = float(fov[1])",
    "    x_offset = -(fov_x / 2.0 - SCALE_BAR_MARGIN_MM - SCALE_BAR_LENGTH_MM / 2.0)",
    "    y_offset = -(fov_y / 2.0 - SCALE_BAR_MARGIN_MM)",
    ...
    "ORIENTCSG_INITIAL_VOLUME_NODE_ID = volumeNode.GetID()",
    ...
    "camera = orient_3d_camera(volumeNode)",
    "",
    ...
    "print('Camera distance factor:', CAMERA_DISTANCE)",
    "print('Red field-of-view height, mm:', BASE_FIELD_OF_VIEW_Y_MM * CAMERA_DISTANCE)",
    "print('3D parallel scale:', BASE_PARALLEL_SCALE_MM * CAMERA_DISTANCE)",
    "print('3D camera oriented:', ORIENT_3D_CAMERA)",
...
emit_slicer_mandible_python <- function(res, section = NULL) {
  ...
  code <- c(
    ...
    "# Display controls",
    sprintf("CAMERA_DISTANCE = %s", fmt_num_py(res$camera_distance)),
    "BASE_FIELD_OF_VIEW_Y_MM = 85.0",
    "BASE_PARALLEL_SCALE_MM = 42.5",
    "CAMERA_POSITION_DISTANCE_MM = 300.0",
    "VOLUME_RENDERING_PRESET = \"CT-AAA2\"",
    "",
    "CREATE_SCALE_BAR = True",
    ...
    "",
    "    view_up = arp_normal",
    "    camera_position = focal_point - CAMERA_POSITION_DISTANCE_MM * view_dir",
    "",
