...
emit_slicer_mandible_python <- function(res, section = NULL) {
  ...
  code <- c(
    ...
    "    z_axis = nrm(z_axis)",
    "",
    "    x_axis = np.asarray(x_reference, dtype=float)",
    "    x_axis = x_axis - dot3(x_axis, z_axis) * z_axis",
    "",
    "    if np.linalg.norm(x_axis) < 1e-12:",
    "        y_tmp = np.asarray(y_preferred, dtype=float)",
    "        y_tmp = y_tmp - dot3(y_tmp, z_axis) * z_axis",
    "        if np.linalg.norm(y_tmp) < 1e-12:",
    "            raise ValueError(\"Cannot construct slice axes: reference vectors are collinear with the slice normal.\")",
    "        x_axis = cross3(y_tmp, z_axis)",
    "",
    "    x_axis = nrm(x_axis)",
    "    y_axis = nrm(cross3(z_axis, x_axis))",
    "    x_axis = nrm(cross3(y_axis, z_axis))",
    "",
    "    y_pref = np.asarray(y_preferred, dtype=float)",
    "    y_pref = y_pref - dot3(y_pref, z_axis) * z_axis",
    "",
    "    if np.linalg.norm(y_pref) > 1e-12:",
    "        if dot3(y_axis, nrm(y_pref)) < 0:",
    "            x_axis = -x_axis",
    "            y_axis = -y_axis",
    "",
    "    m = vtk.vtkMatrix4x4()",
