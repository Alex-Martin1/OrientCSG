...
* Avizo/Amira `flash_capture()` now initializes the first requested section with the current result's Slice orientation, AP/ML visual planes, and standardized orthographic camera before batch capture, preventing orientation from being inherited from a previously displayed analysis.
