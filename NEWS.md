...
* Avizo/Amira `flash_capture()` now initializes the first requested section with the current result's Slice orientation and AP/ML visual planes, then rotates the existing camera perpendicular to that section while preserving camera position, projection type, and zoom/framing. This prevents anatomical orientation from being inherited from a previously displayed analysis without overwriting the user's current framing.
...
* Improved SOLID mesh inertia computation for large meshes by disabling unused normal recomputation and vectorizing signed-tetrahedron integration, substantially reducing computation time while preserving the resulting principal inertia axes.
